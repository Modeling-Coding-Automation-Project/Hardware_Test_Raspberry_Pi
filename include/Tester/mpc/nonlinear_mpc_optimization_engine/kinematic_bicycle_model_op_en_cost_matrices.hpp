...
#include "kinematic_bicycle_model_op_en_cost_matrices_U_max.hpp"
#include "kinematic_bicycle_model_op_en_cost_matrices_U_min.hpp"
#include "kinematic_bicycle_model_op_en_cost_matrices_Y_max.hpp"
#include "kinematic_bicycle_model_op_en_cost_matrices_Y_min.hpp"
...
using Parameter_Type = kinematic_bicycle_model_op_en_parameter::Parameter;

using State_Jacobian_X_Matrix_Type =
    kinematic_bicycle_model_op_en_oe_state_jacobian_x::State_Jacobian_x_Type;
using State_Jacobian_U_Matrix_Type =
    kinematic_bicycle_model_op_en_oe_state_jacobian_u::State_Jacobian_u_Type;
using Measurement_Jacobian_X_Matrix_Type =
    kinematic_bicycle_model_op_en_oe_measurement_jacobian_x::
...
using U_Min_Type = kinematic_bicycle_model_op_en_cost_matrices_U_min::type;
using U_Max_Type = kinematic_bicycle_model_op_en_cost_matrices_U_max::type;
using Y_Min_Type = kinematic_bicycle_model_op_en_cost_matrices_Y_min::type;
using Y_Max_Type = kinematic_bicycle_model_op_en_cost_matrices_Y_max::type;

using Reference_Trajectory_Type =
    DenseMatrix_Type<float, OUTPUT_SIZE, (NP + 1)>;

using type = OptimizationEngine_CostMatrices_Type<
...
inline auto make() -> type {
  ...
  auto U_min = kinematic_bicycle_model_op_en_cost_matrices_U_min::make();

  U_min.template set<0, 0>(static_cast<float>(-1.0));
  U_min.template set<1, 0>(static_cast<float>(-1.5));

  auto U_max = kinematic_bicycle_model_op_en_cost_matrices_U_max::make();

  U_max.template set<0, 0>(static_cast<float>(1.0));
  U_max.template set<1, 0>(static_cast<float>(1.5));

  auto Y_min = kinematic_bicycle_model_op_en_cost_matrices_Y_min::make();

  auto Y_max = kinematic_bicycle_model_op_en_cost_matrices_Y_max::make();
  ...
  type cost_matrices = make_OptimizationEngine_CostMatrices<
      float, NP, Parameter_Type, U_Min_Type, U_Max_Type, Y_Min_Type, Y_Max_Type,
      State_Jacobian_X_Matrix_Type, State_Jacobian_U_Matrix_Type,
